...
use error::{InjectResult, InjectSyntaxError};
use syntax::{
...
pub(crate) fn _macro(attr: TokenStream, item: TokenStream) -> InjectResult<TokenStream> {
    let input =
        syn::parse::<InjectMacroInput>(attr).map_err(InjectSyntaxError::FailedToParseInput)?;
...
impl Default for InjectMacroInput {
    fn default() -> Self {
...
impl Parse for InjectMacroInput {
...
impl InjectMacroInput {
    fn add_self(&self) -> bool {
        match self {
            InjectMacroInput::Static(_) => true,
            InjectMacroInput::Scoped(_) => false,
            InjectMacroInput::Singleton(_) => false,
        }
    }

    fn wrap_type(&self, injectable_ty: Type) -> Type {
        match self {
            InjectMacroInput::Static(_) => injectable_ty,
            InjectMacroInput::Scoped(_) => {
                let injectable_ty = wrap_type(injectable_ty, type_refcell);
                let injectable_ty = wrap_type(injectable_ty, type_rc);
                injectable_ty
            }
            InjectMacroInput::Singleton(_) => {
    ...
    fn wrap_call(&self, constructor_call: Expr) -> Expr {
        match self {
            InjectMacroInput::Static(_) => constructor_call,
            InjectMacroInput::Scoped(_) => {
    ...
    pub(crate) fn factory_prefix(&self) -> &'static str {
        match self {
            InjectMacroInput::Static(_) => FACTORY_PREFIX_STATIC,
            InjectMacroInput::Scoped(_) => FACTORY_PREFIX_SCOPED,
            InjectMacroInput::Singleton(_) => FACTORY_PREFIX_SINGLETON,
        }
    }
}
