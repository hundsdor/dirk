...
use super::{
    error::{InjectLogicError, InjectResult},
    InjectMacroInput,
};

pub(crate) fn get_first_function(input_impl: &ItemImpl) -> InjectResult<ImplItemFn> {
    ...
    let first_function = items
        ...
        .exactly_one()
        .map_err(|e| {
            InjectLogicError::InvalidFunctionCount(
                input_impl.clone(),
                e.try_len()
...
pub(crate) fn get_fields(
    input_impl: &ItemImpl,
) -> InjectResult<(Ident, Punctuated<FnArg, Comma>, Punctuated<Expr, Comma>)> {
    ...
    let actual_fields = formal_fields
        ...
        .collect::<InjectResult<Punctuated<Expr, Comma>>>()?;

    Ok((function_ident, formal_fields, actual_fields))
}

pub(crate) fn get_injectable(input_impl: &ItemImpl) -> InjectResult<(Type, TypePath)> {
    let ty = {
        ...
        if type_path.path.is_ident("Self") {
            ...
            return Err(InjectLogicError::InvalidReturnType(fun_ty))?;
        }
    };
...
pub(crate) fn get_factory_ty(
    input: &InjectMacroInput,
    injectable_ty: &Type,
    params: &Punctuated<GenericParam, Comma>,
) -> InjectResult<(Type, TypePath)> {
